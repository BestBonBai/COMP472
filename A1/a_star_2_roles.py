...
class A_Star_Search:
    ...
    def __init__(self, start_point, end_point, current_map):
        ...
        self.has_path = False # for display the path
        ...
        if self.is_valid_point(aPoint) and self.__is_valid_transform(aPoint):
            ...
            for grid_i in self.current_map.list_grids:
                ...
                if grid_i.x_min < aPoint.x and aPoint.x < grid_i.x_max \
                    ...
                    # check role type
                    if self.current_map.role_type == 'role_c':
                        # transform the top-right point of the grid
                        __x, __y = grid_i.get_top_right_point()
                    elif self.current_map.role_type == 'role_v':
                        # transform the left-bottom point of the grid
                        __x, __y = grid_i.get_bottom_left_point()
                    else:
                        print("[Debug] Invalid Role Type!")
            ...
            if np.round(math.fmod(aPoint.x*10, self.current_map.offsetX*10), 1) == 0.0 and \
                ...
                # check the role type
                if self.current_map.role_type == 'role_c':
                    # round the point's y value: move up
                    ...
                    # check if the point is in the edge of the 2 playing ground, then Notice invalid point
                    if not self.is_accessiable(self.current_map.list_points[self.__get_num_point_in_list(aPoint.x,aPoint.y)], self.current_map.list_points[self.__get_num_point_in_list(aPoint.x,np.round(aPoint.y - self.current_map.offsetY,1))]):
                        print("[Error] Original Point is invalid, it cannot access the edge of 2 playing grounds!")
                        sys.exit(0)
                if self.current_map.role_type == 'role_v':
                    # round the point's y value: move down
                    aPoint.y = np.round((aPoint.y*10 - np.round(math.fmod(aPoint.y*10, self.current_map.offsetY*10), 1))/10,1)
            # case: in the boundary of the row line
            if np.round(math.fmod(aPoint.x*10, self.current_map.offsetX*10), 1) != 0.0 and \
                np.round(math.fmod(aPoint.y*10, self.current_map.offsetY*10), 1) == 0.0:
                # check role type
                if self.current_map.role_type == 'role_c':
                    # round the point's x value: move right
                    ...
                    # check if the point is in the edge of the 2 playing ground, then Notice invalid point
                    if not self.is_accessiable(self.current_map.list_points[self.__get_num_point_in_list(aPoint.x,aPoint.y)], self.current_map.list_points[self.__get_num_point_in_list(np.round(aPoint.x - self.current_map.offsetX,1),aPoint.y)]):
                        print("[Error] Original Point is invalid, it cannot access the edge of 2 playing grounds!")
                        sys.exit(0)
                if self.current_map.role_type == 'role_v':
                    # round the point's x value: move left
                    aPoint.x = np.round((aPoint.x*10 - np.round(math.fmod(aPoint.x*10, self.current_map.offsetX*10), 1))/10,1)
            # find it in the list_points
            for p_i in self.current_map.list_points:
    ...
    def calc_cost_with_2_points(self, current_point, other_point):
        ...
        if len(__list_inter) == 1:
            # check the role type
            # if Role V and the 2 points are diagonal position in the same grid, then using the higher value of the square root
            if self.current_map.role_type == 'role_v':
                # check which direction satisfied
                # North-East
                if np.round(current_point.x - other_point.x,1) == -0.2 and\
                    np.round(current_point.y - other_point.y,1) == -0.1:
                    __square1 = self.calc_cost_with_2_points(current_point, self.current_map.list_points[self.__get_num_point_in_list(np.round(current_point.x+0.2,1),current_point.y)])
                    __square2 = self.calc_cost_with_2_points(self.current_map.list_points[self.__get_num_point_in_list(np.round(current_point.x+0.2,1),current_point.y)], other_point)
                    __square3 = self.calc_cost_with_2_points(current_point, self.current_map.list_points[self.__get_num_point_in_list(current_point.x,np.round(current_point.y+0.1,1))])
                    __square4 = self.calc_cost_with_2_points(self.current_map.list_points[self.__get_num_point_in_list(current_point.x,np.round(current_point.y+0.1,1))], other_point)
                    # print("+"*50)
                    # print(f'[Square] {__square1, __square2, __square3, __square4}')
                    # check the 4 squares cost are valid type
                    if __square1 != None and __square2 != None and __square3 != None and __square4 != None:    
                        return max(np.round(math.sqrt(__square1**2 + __square2**2),1), np.round(math.sqrt(__square3**2 + __square4**2),1))
                    else: return None
                # North-West
                if np.round(current_point.x - other_point.x,1) == 0.2 and\
                    np.round(current_point.y - other_point.y,1) == -0.1:
            ...
            # otherwise, return basic cost of the grid
    ...
    def is_vaccine_area(self, current_point):
        '''
        The method is to check if the point is in the vaccine area or not
        :param: current point
        :return: True or False
        '''
        # get index from the list_points
        ...
        # check if the current_point's belong_num_grids has the quarantine type
        for g_i in  self.current_map.list_points[__current_index].belong_num_grids:
            if self.current_map.list_grids[g_i - 1].type == "place_v":
                return True
        # noice for end point
        if current_point == self.end_point:
            print(f'[Error] End Point {current_point.x, current_point.y} is not in the vaccine area!')
        return False
    ...
    def __push_open_list(self, current_point, other_x, other_y):
        ...
        if __num != None:
            ...
            if __cost != None:
                ...
                if self.current_map.role_type == 'role_c' and \
                    self.is_accessiable(self.current_map.list_points[__c_index], self.current_map.list_points[__num]):
                    pass             
                elif self.current_map.role_type == 'role_v':
                    pass
                else:
                ...
                if self.__is_in_open_list(self.current_map.list_points[__num]):
                    ...
                    # compare the f_cost of the point which in the openList
                    if __point_f_cost < self.current_map.list_points[__num].f_cost:
                        ...
                        self.current_map.list_points[__num].g_cost = np.round(__point_g_cost,1)
                        self.current_map.list_points[__num].h_cost = np.round(__point_h_cost,1)
                        self.current_map.list_points[__num].f_cost = np.round(__point_f_cost,1)
                        # test
                ...
                if not self.__is_in_close_list(self.current_map.list_points[__num]) \
                    ...
                    self.current_map.list_points[__num].g_cost = np.round(__cost + self.current_map.list_points[__num].parent.g_cost,1)
                    # calculate h_cost
                    self.current_map.list_points[__num].h_cost = np.round(self.__calc_h_cost(self.current_map.list_points[__num]),1)
                    # calculate the f_cost
                    self.current_map.list_points[__num].f_cost = np.round(self.current_map.list_points[__num].h_cost + self.current_map.list_points[__num].g_cost,1)
    ...
    def __calc_h_cost(self, current_point):
        ...
        Role C --- Use Manhattan distance: abs(point.x - goal.x) + abs(point.y - goal.y)
        Role V --- Use Chebyshev distance: max(|point.x-goal.x|, |point.y - goal.y|)
        '''
        # Check the role type
        if self.current_map.role_type == 'role_c':
        ...
        # Thus, it will make sure the steps of the points are equal in 4 different directions.
            ...
            # consider the diagonal directions, the (point.x - goal.x)/2 to let the steps are equal in 8 directions
            __h_cost = 10 * max(np.round(abs((current_point.x - self.goal_point.x)/2),1), np.round(abs(current_point.y - self.goal_point.y),1) )
        return __h_cost
        ...
        for i_p in range(len(self.current_map.list_points)):
            ...
            if self.current_map.role_type == 'role_c':
                if self.is_quarantine_area(self.current_map.list_points[i_p]):
                    ...
                    __temp_num += 1
            if self.current_map.role_type == 'role_v':
                if self.is_vaccine_area(self.current_map.list_points[i_p]):
                    self.goal_points_list.append(self.current_map.list_points[i_p])
                    __temp_num += 1
    ...
    def run_search(self,goal_point):
        ...
        '''  
        # (4) (Role C) check the end point is in the quarantine place 
        if (self.current_map.role_type == 'role_c' and self.is_quarantine_area(goal_point)) \
            or (self.current_map.role_type == 'role_v' and self.is_vaccine_area(goal_point)):
            print("-"*50)
            ...
            while self.open_list:
                ...
                # check the role type
                if self.current_map.role_type == 'role_c' and \
                    (current_node == goal_point or self.is_quarantine_area(current_node)):
                    ...
                    self.__get_path()
                    break 
                if self.current_map.role_type == 'role_v' and \
                    (current_node == goal_point or self.is_vaccine_area(current_node)):
                    # check if current node reached to the goal node
                    # or the current node is already in the vaccine area
                    # Recall fuction to return "reverse" close list as path list
                    print("[News] Found the path!") 
                    self.__get_path()
                    break 
                ...
                # then push in the openlist
                print("-"*50)
                # order is 'up, down, left, right' respectively (Role C)
                print("[Searching] Basic 4 directions ('up, down, left, right'), if the point is in the close list, then not search!")
                print("[Searching] If the point is already in the open list, compare its f_cost with the old one.\n            If the new one is less than old one, then update its f_cost and parent node.")
                ...
                self.__push_open_list(current_node, np.round(current_node.x + self.current_map.offsetX,1), current_node.y)
                print("[Completed] Have searched basic 4 directions!")
                # (Role V) add 4 diagonal directions for Role V
                if self.current_map.role_type == 'role_v':
                    # search 4 diagonal directions ('N-E: North-East','N-W: North-Weat', 'S-E: Sorth-East', 'S-W: Sourth West')
                    print("[Searching] 4 diagonal directions ('N-E: North-East','N-W: North-West', 'S-E: Sorth-East', 'S-W: Sourth West'), if the point is in the close list, then not search!")
                    print("[Searching] If the point is already in the open list, compare its f_cost with the old one.\n            If the new one is less than old one, then update its f_cost and parent node.")
                    self.__push_open_list(current_node, np.round(current_node.x + self.current_map.offsetX,1), np.round(current_node.y + self.current_map.offsetY,1))
                    self.__push_open_list(current_node, np.round(current_node.x - self.current_map.offsetX,1), np.round(current_node.y + self.current_map.offsetY,1))
                    self.__push_open_list(current_node, np.round(current_node.x + self.current_map.offsetX,1), np.round(current_node.y - self.current_map.offsetY,1))
                    self.__push_open_list(current_node, np.round(current_node.x - self.current_map.offsetX,1), np.round(current_node.y - self.current_map.offsetY,1))
                    print("[Completed] Have searched 8 directions!")
                print("="*50)
    ...
    def start(self):
        ...
        if self.is_valid_point(self.start_point) and self.is_valid_point(self.end_point):
            ...
            # (3) check the start point is already in the quarantine place or vaccine place
            if self.current_map.role_type == 'role_c':
    ...
    def __find_optimal_path(self):
        ...
        self.optimal_path_total_cost = np.round(self.optimal_path_node.total_cost,1)
        print("-"*50)
        print("[Optimal Path]")
        print(f'{self.optimal_path}')
        print(f'[Total Cost] {self.optimal_path_total_cost}')
        print("-"*50)
        # set has_path is true
        self.has_path = True
